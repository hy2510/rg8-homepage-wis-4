...
const DodoAndFriends = () => {
  ...
  const dodofriends = getDodoFriendsData(language)
  ...
  if (point < 50000) {
    ...
    currentPoint = NumberUtils.toRgDecimalPoint(point)
    if (findIndex >= 0) {
      let minPoint = 0
      let range = 0
  ...
  useEffect(() => {
    if (modalContentRef.current) {
      ...
      const elements = modalContentRef.current.querySelectorAll(
        '.scroll-check-point',
      )
  ...
  return (
    <>
      <div className={style.quest_modal_body}>
        <div
          className={style.dodo_and_friends}
          ref={modalContentRef}
            ...
            return (
              <div key={friend.id}>
                ...
                {point > friend.minPoint && (
                  <div className="scroll-check-point">
                    <FriendsEntry
                      imgSrc={
                        point < friend.minPoint
    ...
    <>
      ...
      <div className={style.status}>
        <div className={style.comment}>
          ...
          <span>
            {NumberUtils.toRgDecimalPoint(currentEarnPoint)}/{goalPoint}P
          </span>
        </div>
...
const LevelMasterProgressItem = ({
  ...
  const progress = NumberUtils.getHundredPercentage(currentEarnPoint, goalPoint)
