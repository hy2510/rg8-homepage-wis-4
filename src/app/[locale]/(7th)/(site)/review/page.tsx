...
import ReadingList from './_cpnt/ReadingList'
...
function HistoryLayout({ initLoading }: { initLoading?: boolean }) {
  ...
  const onSearch = (startDt: string, endDt: string, inKeyword: string) => {
    if (inKeyword) {
      ...
      if (isChange) {
        router.push(`${SITE_PATH.REVIEW.MAIN}?keyword=${inKeyword}`)
      ...
      if (isChangeDate) {
        ...
        router.push(
          `${SITE_PATH.REVIEW.MAIN}?startDate=${date}&endDate=${enddate}`,
        )
  ...
  const onMoreHistory = () => {
    ...
    if (!isReportLoading && page < totalPages) {
  ...
  return (
    <>
      ...
      />
      {!initLoading && (
        <ReadingList isReportLoading={isReportLoading} onMore={onMoreHistory} />
      )}
    </>
  )
}
