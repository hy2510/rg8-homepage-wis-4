...
import { useSearchBookTheme } from '@/8th/features/library/service/search-query'
import BookItem, {
  SkeletonBookItem,
...
export default function SearchThemeBookList({
  booktype,
  code,
  name,
}: {
  ...
  if ((booktype !== 'EB' && booktype !== 'PB') || !code || !name) {
    return (
      <>
        Invalid params {booktype}-{code}-{name}
      </>
    )
  }
...
function LibraryBookList({
  ...
  useEffect(() => {
    maketingEventTracker.eventAction('도서 검색', {
      version: '8th',
      section_name: 'Theme Book',
      book_type: bookType === 'EB' ? 'eBook' : 'p Book Quiz',
      theme_name: name,
    })
  }, [maketingEventTracker, bookType, name])
  ...
  const search = useSearchBookTheme(themeSearch)
  ...
  const sortItems = useSortItems(themeSearch.sort)
  ...
  const onStatusItemClick = (item: ActionBarDropdownItem) => {
    if (item.key.startsWith('Status')) {
      ...
      if (value.startsWith('Mode')) {
        ...
        setThemeSearch({ ...themeSearch, status, page: 1 })
      } else if (themeSearch.status !== value) {
        setThemeSearch({
          ...themeSearch,
          status: value,
          page: 1,
      ...
      if (themeSearch.genre !== value) {
        setThemeSearch({ ...themeSearch, genre: value, page: 1 })
      }
    }
  }
  ...
  const onSortItemClick = (item: ActionBarDropdownItem) => {
    ...
    setThemeSearch({ ...themeSearch, sort: value })
  }
  ...
  const onPageClick = (page: number) => {
    setThemeSearch({ ...themeSearch, page: page })
  }
