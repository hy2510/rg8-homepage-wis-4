...
import { useSearchBookWorkbook } from '@/8th/features/library/service/search-query'
import BookItem, {
  SkeletonBookItem,
...
export default function SearchWorkBookList({
  booktype,
  level,
  volume,
}: {
...
function LibraryBookList({
  ...
  const search = useSearchBookWorkbook(workbookSearch)
  ...
  const onLevelChange = (selectLevel: string) => {
    ...
    //   page: 1,
    // })
    router.replace(`${SITE_PATH.NW82.EB_WORKBOOK}/${selectLevel}?volume=1`)
  }

  const onVolumeChange = (selectVolume: number) => {
    if (selectVolume === volume) {
      return
    }
    setWorkbookSearch({
      ...workbookSearch,
    ...
    router.replace(
      `${SITE_PATH.NW82.EB_WORKBOOK}/${level}?volume=${selectVolume}`,
    )
  }

  const actionBarTitle = t('t8th040')
  ...
  const onPageClick = (page: number) => {
    setWorkbookSearch({ ...workbookSearch, page: page })
  }
  ...
  return (
    <>
      ...
      <SelectBox
        ...
        largeFont
        onChange={(volume) => onVolumeChange(parseInt(volume.key))}
        options={volumes.map((item) => ({ key: item, label: item }))}
      />
    </>
  )
}
