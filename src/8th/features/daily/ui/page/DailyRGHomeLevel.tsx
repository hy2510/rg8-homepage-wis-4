...
export default function RGTrackHomeLevel({ level }: { level?: string }) {
  ...
  const onDailyLevelChange = (level: string) => {
    router.replace(`${SITE_PATH.NW82.DAILY_RG}?level=${level}`)
  }
  ...
  const onDailyStageChange = (stageId: string) => {
    ...
    router.replace(`${SITE_PATH.NW82.DAILY_RG}?level=${targetLevel}`)
  }
  ...
  const { mutate: changeStudyLevel, isPending: isChangeStudyLevelPending } =
    useChangeStudyLearningLevel({
      onSuccess: (data, variables) => {
        router.replace(`${SITE_PATH.NW82.DAILY_RG}?level=${variables}`)
      },
    })
            ...
            return (
              <Fragment key={section.sectionId}>
                <DailyRGSection
                  ...
                  onSectionClick={() => {
                    router.push(
                      `${SITE_PATH.NW82.DAILY_RESULT}?level=${targetLevel}&stage=${stageId}&section=${section.sectionId}`,
                    )
                  }}
                />
