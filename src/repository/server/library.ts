  ...
  const request = makeRequest({
    ...
    option: {
      ...
      queryString: {
        courseCode: '062001',
        studyTypeCode: input.studyTypeCode,
        searchType: input.searchType || 'LibrarySearchAll',
        searchText: encodeURIComponent(input.searchText),
        sort: 'Round',
        page: input.page || 1,
      },
    },
  ...
  const request = makeRequest({
    ...
    option: {
      ...
      queryString: {
        ...
        studyTypeCode: '001006',
        level: input.level,
        searchText: encodeURIComponent(input.keyword),
        searchType: 'WorkBook',
        sort: 'Round',
        status: 'All',
        genre: 'All',
...
async function categoryContinue(token: string) {
  const request = makeRequest({
    token,
    path: getPath('category-continue'),
    option: {
      method: 'get',
      queryString: {},
    },
